"use strict";
/* Levi Noell-Baba, October 15, 2023, IST239-W01, Hands-on project 08-05
   
   In Hands-on exercise 08-05 the book is teaching us to correctly detect and correct errors in code.
 */

// Constructor Function for Chess Piece
function piece(color, rank) {
   this.color = color;    // the color of the piece (white or black) //this added before color Error #1
   this.rank = rank;      // the piece rank (Pawn, Knight, Bishop, Rook, Queen, King) //this added before rank Error #2
   this.square = null;    // the square the piece occupies //this added before square Error #3
   this.image = null;     // stores the HTML code representing the image of the piece //this added before image Error #4
}
   ...
   for (let i = 0; i < 8; i++) {         // loop through the chess board rows
      for (let j = 0; j < 8; j++) {      // loop through the chess board columns
         if (game.board[i][j].length === 2) {
            ...
            let chessPiece = new piece(color, rank); //Piece changed to piece Error #5
